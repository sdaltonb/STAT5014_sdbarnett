...
kable(summary(sensory_data_base_r))
...
kable(summary(gold_medal_data_frame))
...
kable(summary(brain_body_weight_data_frame))
...
kable(summary(tomato_yield_data_frame))
ggplot(data = tomato_yield_data_frame, aes(factor(PlantDensity), Yield, fill = TomatoVariety)) + geom_bar(stat = "identity", width = 0.5, position = "dodge") + labs(list(x = "x", y = "count", fill = "group")) + xlab("Plant Density") + ylab("Highest Measured Yield")
```